...
- [ ] Inside `utils/markdownParser.js` in your client directory, write a module that implements a basic Markdown parser. This module should export a function that accepts a string of Markdown-formatted text, and returns that same text but converted into HTML formatting.
  - **DO NOT** use or reference the docs/source code/npm modules for any existing Markdown Parsing module.
...
Feel free to reach a minimal function first - you may move on to other steps before returning to finish your parser but note that it is required to complete later sections. **WHEN THIS STEP IS COMPLETE:** please make a commit with the message "complete step one"

---

### Step Two: The Blog Feed View
...
**Implement this user story by doing the following:**
  ...
  - [ ] Study the comments explaining the workings of the "view switcher" in the client-side's `App` component.
  ...
  - [ ] Use the [Moment.js](https://momentjs.com) library to display the post dates using relative time - for example, "21 hours ago" or "10 minutes ago."
  - [ ] Refactor the `Post` component to make use of your Markdown Parser module. Ensure that your Markdown parser is [not susceptible to XSS attacks](https://reactjs.org/docs/dom-elements.html#dangerouslysetinnerhtml).

**WHEN THIS STEP IS COMPLETE:** please make a commit with the message "complete step three"

---

### Step Four: The Admin View, and Tracking Views on our Blog Post
...
- [ ] Create a new `Admin` component, which will render a list of blog posts showing. Each item in the list should display the blog post's *title, author, and `views` count*.
  - To aid you in crafting the structure, styling, and layout of the HTML elements in this component, we've provided an HTML "skeleton" for this component. You can find it at `example_templates/Admin.html`.
  - You may create or refactor other components as necessary.
...
- [ ] Create a new `Create` component, which will display a form with several inputs: one input each for the _title, author, and imageUrl_ of the post, a `textarea` for the _body_ of the post, and a `button` that will submit the post. Additionally, the `Create` component will display a preview of the blog post.
  - To aid you in crafting the structure, styling, and layout of the HTML elements in this component, we've provided an HTML "skeleton" for this component. You can find it at `example_templates/Create.html`.
...
**WHEN THIS STEP IS COMPLETE:** plase make a commit with the message "complete step five"
