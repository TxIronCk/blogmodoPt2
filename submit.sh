...
# Updated by Joseph Martin <joseph.martin@galvanize.com>
#
...
# Input cohort prefix
read -p "${MAGENTA}Enter your cohort's prefix (e.g. hr-atx99):${COLORLESS} " COHORT_PREFIX

if ! [[ $COHORT_PREFIX =~ $PREFIX_REGEX ]]; then
  echo "${RED}ERROR:${COLORLESS} Invalid prefix: ${COHORT_PREFIX}"
  echo "${YELLOW}Please enter a prefix that follows the convention hr-camp##${COLORLESS} (e.g. hr-nyc88)"
  exit 1
fi
...
echo "Please contact a proctor about granting you permission to the following repo:"
...
# if ! [[ `git diff $REPO_REMOTE_ALIAS/$REPO_BASE_BRANCH -- closing_comments` ]]; then
#   echo "${RED}ERROR:${COLORLESS} Closing comments not found."
#   echo "${YELLOW}Please fill out and commit your changes to the closing_comments file and try again.${COLORLESS}"
#   exit 1
# fi
